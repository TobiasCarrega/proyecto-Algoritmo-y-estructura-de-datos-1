...
def guardar_json(datos: dict, nombre_archivo: str):
    """
    Objetivo: Guarda un diccionario Python en un archivo JSON.
    Entrada:
        - datos (dict): Diccionario que se va a guardar.
        - nombre_archivo (str): Archivo destino.
    """
    try:
        with open(nombre_archivo, 'w', encoding='utf-8') as archivo:
            json.dump(datos, archivo, indent=4, ensure_ascii=False)
        print(f"Entidad '{nombre_archivo}' registrada en JSON correctamente.")
    except Exception as e:
        print(f"Error al registrar la entidad en '{nombre_archivo}': {e}")
...
def generar_archivos_iniciales():
    """
    ...
    try:
        ...
        guardar_json(talles_inicial, "talles.json")
...
def alta_cliente(clientes: dict):
    ...
    guardar_json(clientes, "clientes.json")
...
def alta_accesorio(accesorios: dict):
    ...
    accesorios = cargar_json("accesorios.json")
    ...
    guardar_json(accesorios, "accesorios.json")
...
def alta_talle(talles: dict):
    ...
    """
    print(">>> ALTA TALLE")
    talles = cargar_json("talles.json")
    ...
    guardar_json(talles, "talles.json")
...
#----------------------------------------------------------------------------------------------
def main():
    print("Inicio de registro de datos en JSON.")
    # Genera los archivos iniciales
    generar_archivos_iniciales()
        ...
        if opcionMenuPrincipal == "0":
            ...
            while alquileres_running:
                ...
                if opcionSub == "0":
                    ...
                    pausar()
                    print("\n\n")
# Guardamos los cambios al finalizar
    guardar_json(clientes, "clientes.json")
    guardar_json(accesorios, "accesorios.json")
    guardar_json(talles, "talles.json")
